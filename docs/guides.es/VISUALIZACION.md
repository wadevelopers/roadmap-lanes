...
(`parent`, `depends_on`, `absorbs`) y la clasificación como **propiedades** (`type`, `status`,
`maturity`, `areas`, `zones`). El grafo nativo y los plugins leen exactamente esos campos.
...
El **nombre de la propiedad es el tipo de link**: `parent`, `depends_on` y `absorbs` se vuelven tipos
