...
(`parent`, `depends_on`, `absorbs`, `part_of`) y la clasificación como **propiedades** (`type`,
`status`, `maturity`, `areas`, `zones`). El grafo nativo y los plugins leen exactamente esos campos.
...
```text
[type:feat]            # colorear por tipo: features…
[type:maint]            # …deuda técnica…
[type:infra]         # …infraestructura
[type:combo]         # …agrupadores/etapas con hijos
[type:doc]           # …partes (documentos acompañantes de una tarea)
[status:done]       # o enfocar por estado
[maturity:raw]       # o por madurez del plan
```

Sintaxis útil de filtros:

- `[parent]` — notas que **tienen** la propiedad `parent`; `-[parent]` — las que **no** (raíces del árbol).
- `[parent:null]` — `parent` declarado pero vacío.
- Grupos compuestos: `[type:maint] [status:pending]`.

> No conviene crear grupos para **todas** las combinaciones de `type` × `status` × `maturity`: la
...
1. **No conoce el significado de cada link.** Para Obsidian, un link en `parent` y uno en `depends_on`
   son lo mismo: no etiqueta la línea como "padre", "depende de" o "absorbe".
2. **Una sola dimensión de color a la vez.** No puede mostrar `type` + `status` + `maturity` juntos, y
...
El **nombre de la propiedad es el tipo de link**: `parent`, `depends_on`, `absorbs` y `part_of` se
vuelven tipos distintos automáticamente. Con eso se puede:

- **colorear cada tipo** (p. ej. `parent` azul, `depends_on` rojo, `absorbs` gris) y mostrar su
  etiqueta sobre la línea;
- **filtrar/ocultar por tipo** — ver, por ejemplo, **solo** el árbol de jerarquía (`parent`) o **solo**
  las dependencias (`depends_on`).
...
la vez** como anillos concéntricos. Así se ve `type` + `status` + `maturity` **simultáneamente**, sin
...
Se puede dimensionar cada nodo por una **propiedad numérica**. Apuntándolo a **`duration`** (que en RL
es un **número de horas, sin sufijo**), **un nodo más grande = más trabajo**: de un vistazo se ve qué
tareas pesan más. *(Esta es una de las razones por las que `duration` se declara como número plano y
...
Para vistas **tabulares** del roadmap (filtrar por `type`/`status`/`area`, agrupar, ordenar),
